...
import { encodeMessage, MessageDecoder, RawMessageAccumulator, LENGTH_PREFIX_SIZE, MAX_MESSAGE_SIZE_TO_CHROME, extractPayload, encodeRawWithPrefix, } from '../dist/protocol.js';
...
describe('RawMessageAccumulator', () => {
    ...
    it('should preserve length prefix in returned frames', () => {
        ...
        assert.strictEqual(frames[0].readUInt32LE(0), encoded.length - LENGTH_PREFIX_SIZE);
    });
...
describe('encodeRawWithPrefix', () => {
    it('should add length prefix to raw data', () => {
        ...
        assert.strictEqual(Buffer.compare(encoded.subarray(LENGTH_PREFIX_SIZE), data), 0);
    });
});
